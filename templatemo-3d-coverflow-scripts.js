...
// JavaScript Document
        ...
        function getProducts() {
            try {
                const raw = localStorage.getItem('nordic_products');
        ...
        /* ====== Basit (Demo) Kullanıcı Kayıt/Giriş ====== */
        ...
        function getUsers() {
            try {
                const raw = localStorage.getItem('nordic_users');
                return raw ? JSON.parse(raw) : [];
            } catch { return []; }
        }
        function saveUsers(list) {
            localStorage.setItem('nordic_users', JSON.stringify(list));
        }
        function getCurrentUser() {
            try {
                const raw = localStorage.getItem('nordic_current_user');
        ...
        signupCancel?.addEventListener('click', () => hide(signupModal));
        ...
        loginSubmit?.addEventListener('click', () => {
            ...
            if (!u) return alert('Hatalı e‑posta veya şifre.');
            setCurrentUser({ email: u.email, name: u.name, role: u.role || 'user' });
            hide(loginModal);
            updateAuthUI();
        });
        ...
        signupSubmit?.addEventListener('click', () => {
            ...
            if (!name || !email || !pass) return alert('Lütfen tüm alanları doldurun.');
            if (users.some(x => x.email === email)) return alert('Bu e‑posta ile kayıt mevcut.');
            const role = users.length === 0 ? 'admin' : 'user'; // ilk kullanıcıyı admin yap
        ...
        loadCart();
        updateCartCount();
        renderProducts(getSortedFiltered());
        renderCart();
        updateAuthUI();
